...
cells = [
    markdown_cell(
        """
# V6 Inference-Only LightGBM Submission
...
- A separate Kaggle Dataset or Model containing the local v6 artifact directory with:
  - `v5_manifest.json`
  - `model.joblib`
  - `preprocess.json`
  - `selected_polars_columns.json`

The notebook builds test features, including `credit_bureau_a_2` two-stage aggregation when requested by the manifest, writes the selected matrix to disk, releases memory, then predicts in chunks.
"""
    ),
    code_cell(
        """
...
WORKING = Path("/kaggle/working") if Path("/kaggle/working").exists() else Path("outputs/kaggle_v6")
...
out_path = Path("submission/v6_inference_only.ipynb")

...
cells = [
    markdown_cell(
        """
# V8 Inference-Only LightGBM Submission
...
- A separate Kaggle Dataset or Model containing the local v8 artifact directory with:
  - `v5_manifest.json`
  - `model.joblib`
  - `preprocess.json`
...
WORKING = Path("/kaggle/working") if Path("/kaggle/working").exists() else Path("outputs/kaggle_v8")
...
out_path = Path("submission/v8_inference_only.ipynb")
